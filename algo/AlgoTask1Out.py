...
class AlgoFunctions():
    ...
    def count_repeated_commands(action): 
        ...
        for command in action:    
            ...
            if command in ['FS', 'BS'] and command == prev_command: 
                count += 1 
            else: 
    ...
    def AStarSearch(grid, start, target, object_positions, obs_id, timeout=60):
        ...
        def is_valid_front_turn(x, y, direction, orientation):
            ...
            if direction == 'L':
                # Move up by 1
                if not is_valid_move(new_x, new_y, 'FS', orientation, grid):
                    return False
        ...
        def is_valid_back_turn(x, y, direction, orientation):
            ...
            if direction == 'R':
                ...
                if not is_valid_move(new_x, new_y, 'FS', orientation, grid):
                    return False
                for i in range(2):
                    if not is_valid_move(new_x, new_y, 'BS', orientation, grid):
                        return False
        ...
        def get_successors(state, object_positions):
            ...
            if state.direction == 'N':
                if is_valid_move(state.x, state.y, 'FS', state.direction, grid):
                    successors.append(
                        State(state.x - 1, state.y, state.direction, 'FS', state))
                ...
                if is_valid_move(state.x, state.y, 'FS', state.direction, grid):
                    successors.append(
                        State(state.x, state.y + 1, state.direction, 'FS', state))
            ...
            if state.direction == 'N':
                if is_valid_move(state.x, state.y, 'BS', state.direction, grid):
                    successors.append(
                        State(state.x + 1, state.y, state.direction, 'BS', state))
            elif state.direction == 'S':
                if is_valid_move(state.x, state.y, 'BS', state.direction, grid):
                    successors.append(
                        State(state.x - 1, state.y, state.direction, 'BS', state))
                ...
                if is_valid_move(state.x, state.y, 'BS', state.direction, grid):
                    successors.append(
                        State(state.x, state.y - 1, state.direction, 'BS', state))
            elif state.direction == 'W':
                if is_valid_move(state.x, state.y, 'BS', state.direction, grid):
                    successors.append(
                        State(state.x, state.y + 1, state.direction, 'BS', state))
            ...
            if is_valid_front_turn(state.x, state.y, 'L', state.direction):
                if state.direction == 'N':
                    successors.append(State(state.x-1, state.y-4, get_new_direction(state.direction, 'L', 'FS'), 'FL', state))
                elif state.direction == 'S':
                    successors.append(State(state.x+1, state.y+4, get_new_direction(state.direction, 'L', 'FS'), 'FL', state))
                elif state.direction == 'E':
                    successors.append(State(state.x-4, state.y+1, get_new_direction(state.direction, 'L', 'FS'), 'FL', state))
                elif state.direction == 'W':
                    successors.append(State(state.x+4, state.y-1, get_new_direction(state.direction, 'L', 'FS'), 'FL', state))
            ...
            if is_valid_front_turn(state.x, state.y, 'R', state.direction):
                if state.direction == 'N':
                    successors.append(State(state.x-1, state.y+4, get_new_direction(state.direction, 'R', 'FS'), 'FR', state))
                elif state.direction == 'S':
                    successors.append(State(state.x+1, state.y-4, get_new_direction(state.direction, 'R', 'FS'), 'FR', state))
                elif state.direction == 'E':
                    successors.append(State(state.x+4, state.y+1, get_new_direction(state.direction, 'R', 'FS'), 'FR', state))
                elif state.direction == 'W':
                    successors.append(State(state.x-4, state.y-1, get_new_direction(state.direction, 'R', 'FS'), 'FR', state))
            ...
            if is_valid_back_turn(state.x, state.y, 'L', state.direction):
                if state.direction == 'N':
                    successors.append(State(state.x+4, state.y-1, get_new_direction(state.direction, 'L', 'BS'), 'BL', state))
                elif state.direction == 'S':
                    successors.append(State(state.x-4, state.y+1, get_new_direction(state.direction, 'L', 'BS'), 'BL', state))
                elif state.direction == 'E':
                    successors.append(State(state.x-1, state.y-4, get_new_direction(state.direction, 'L', 'BS'), 'BL', state))
                elif state.direction == 'W':
                    successors.append(State(state.x+1, state.y+4, get_new_direction(state.direction, 'L', 'BS'), 'BL', state))
            ...
            if is_valid_back_turn(state.x, state.y, 'R', state.direction):
                if state.direction == 'N':
                    successors.append(State(state.x+4, state.y+1, get_new_direction(state.direction, 'R', 'BS'), 'BR', state))
                elif state.direction == 'S':
                    successors.append(State(state.x-4, state.y-1, get_new_direction(state.direction, 'R', 'BS'), 'BR', state))
                elif state.direction == 'E':
                    successors.append(State(state.x+1, state.y-4, get_new_direction(state.direction, 'R', 'BS'), 'BR', state))
                elif state.direction == 'W':
                    successors.append(State(state.x-1, state.y+4, get_new_direction(state.direction, 'R', 'BS'), 'BR', state))
            return successors
        ...
        def heuristic(current, state, target):
            ...
            if state.action == 'FS':
                if current is not None and current.action == 'BS':
                    return distance + 1000
                return distance + forward_penalty
            if state.action == 'BS':
                if current is not None and current.action == 'FS':
                    return distance + 1000
        ...
        while priority_queue:
            ...
            if current_node.state == target_state:
                ...
                if current_node.parent:
                    ...
                    action_mapping = {
                        ((1, 0), 'N', 'N'): 'BS', ((-1, 0), 'S', 'S'): 'BS',
                        ((0, -1), 'E', 'E'): 'BS', ((0, 1), 'W', 'W' ): 'BS',
                        ((-1, 0), 'N', 'N'): 'FS', ((1, 0), 'S', 'S'): 'FS',
                        ((0, 1), 'E', 'E'): 'FS', ((0, -1), 'W', 'W'): 'FS',
            ...
            for successor_state in successors:
                ...
                if current_node.parent and current_node.parent.action == 'FS' and successor_node.action == 'BS':
                    successor_node.repeated_move = True
                elif current_node.parent and current_node.parent.action == 'BS' and successor_node.action == 'FS':
                    successor_node.repeated_move = True
